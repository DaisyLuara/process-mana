  ...
  <div class="add-perms-wrap">
    <div v-loading="setting.loading" :element-loading-text="setting.loadingText">
      ...
      <el-form ref="permsForm" :model="permsForm" label-width="120px" class="permsForm">
          ...
          <el-table :data="allPerms" border ref="permsTable" class="perms-table">
            <el-table-column label="一级">
              <template slot-scope="scope">
                <el-checkbox-group @change="handleChange(scope.row)" v-model="permsForm.perms">
        ...
        <el-form-item>
          <el-button type="primary" @click="onSubmit('permsForm')">保存</el-button>
...
export default {
  ...
  methods: {
    selectChildPerm(checkedPerm, type) {
      let subPerm = [],
        checkedPerms = this.permsForm.perms;
    ...
    selectParentPerm(parentPerm, permsName, times) {
      let checkedPerms = this.permsForm.perms;
    ...
    handleChange(checkedPerm) {
      let checkedPerms = this.permsForm.perms;
      if (this.permsForm.perms.includes(checkedPerm.id)) {
        // 选中: 所有子权限
    ...
    onSubmit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          // this.loading = true;
          // saveUser(this, this[formName].user, this.permsID)
          //   .then(result => {
          //     this.loading = false;
          //     this.$message({
          //       message: this.permsID ? "修改成功" : "添加成功",
          //       type: "success"
          //     });
          //     // todo是否返回用户列表
          //     this.$router.push({
