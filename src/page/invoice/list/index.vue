          ...
          <el-table-column label="操作" min-width="280">
            <template slot-scope="scope">
              ...
              <el-button
                ...
                size="mini"
                @click="auditingInvoice(scope.row)"
              >审批</el-button>
              <el-button
              ...
              >删除</el-button>
              <el-button
                v-if="scope.row.status === '已开票' && finance"
                size="mini"
                type="warning"
  ...
  methods: {
    ...
    receiveInvoice(data) {
      ...
      this.$confirm("确认认领此票据?", "提示", {
        ...
        .then(() => {
          this.setting.loadingText = "确认认领此票据中";
          this.setting.loading = true;
          receiveInvoice(this, id)
            .then(response => {
    ...
    editInvoice(data) {
      this.$router.push({
        path: "/invoice/list/edit/" + data.id
      });
    },
    auditingInvoice(data) {
      this.$router.push({
        path: "/invoice/list/detail/" + data.id
      });
    },
    detailInvoice(data) {
      this.$router.push({
